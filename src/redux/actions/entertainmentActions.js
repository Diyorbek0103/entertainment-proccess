...
export const getTrending = (page) => async (dispatch) => {
  ...
  try {
    const res = await axios.get(
      `${PATH_NAME}/trending/all/week?api_key=${APIKey}&page=${page}`
    );
    dispatch({
      type: GET_TRENDING,
      payload: res.data,
...
export const getSingleCinema = (cinema_type, cinema_id) => async (dispatch) => {
  ...
  try {
    const res = await axios.get(
      `${PATH_NAME}/${cinema_type}/${cinema_id}?api_key=${APIKey}&language=en-US`
    );
...
export const getSingleCinemaVideo =(cinema_type, cinema_id) => async (dispatch) => {
    ...
    try {
      const res = await axios.get(
        `${PATH_NAME}/${cinema_type}/${cinema_id}/videos?api_key=${APIKey}&language=en-US`
      );
      dispatch({
...
export const getSingleCinemaCredits =(cinema_type, cinema_id) => async (dispatch) => {
    ...
    try {
      const res = await axios.get(
        `${PATH_NAME}/${cinema_type}/${cinema_id}/credits?api_key=${APIKey}&language=en-US`
      );
      dispatch({
...
export const getMovies =(page,genresForUrl) => async (dispatch) => {
    ...
    try {
      const res = await axios.get(
        `${PATH_NAME}/discover/movie?api_key=${APIKey}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${page}&with_genres=${genresForUrl}`
      );
      dispatch({
        type: GET_MOVIES,
        payload: res.data,
  ...
  export const getMoviesGenresData =(cinema_type) => async (dispatch) => {
    ...
    try {
      const res = await axios.get(
        `${PATH_NAME}/genre/${cinema_type}/list?api_key=${APIKey}&language=en-us`
      );
      dispatch({
